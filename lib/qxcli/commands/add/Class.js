 ...
 * 
 */
qx.Class.define("qxcli.commands.add.Class", {
    ...
    getYargsCommand: function() {
      return {
        ...
        handler: function(argv) {
          return new qxcli.commands.add.Class(argv)
            .process()
            .catch((e) => {
  ...
  members: {
    process: async function() {
      ...
      if( file_exists && ! argv.force){
        throw new qxcli.Utils.UserError(`Class file ${relative_path} already exists. Use --force to overwrite it` );
      } 
      ...
      try {
        ...
        throw new qxcli.Utils.UserError(`Cannot write to ${absolute_path}: ${e.message}` );
      }
      ...
      if( argv.import ){
        ...
        try {
          ...
          throw new qxcli.Utils.UserError(`Cannot copy template to ${local_templates_path}: ${e.message}` );
        }
      }      
    }
  }
