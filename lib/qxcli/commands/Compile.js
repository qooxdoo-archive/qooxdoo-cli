...
qx.Class.define("qxcli.commands.Compile", {
  ...
  members: {
    ...
    parse: async function() {
      ...
      var parsedArgs = await this.parseImpl();
      var config = {};
      var contribConfig = {};
      if (parsedArgs.config) {
        ...
        this.mergeArguments(parsedArgs, config, contribConfig);
      } else {
        this.mergeArguments(parsedArgs, config, contribConfig);
      }
      if (config.libraries) {
    ...
    loadConfig: async function(path) {
      ...
      async function loadJs(path, inputData) {
        var src = await fs.readFileAsync(path, { encoding: "utf8" });
    ...
    __loadJson: async function(path) {
      var data = await fs.readFileAsync(path, { encoding: "utf8" });
      try {
