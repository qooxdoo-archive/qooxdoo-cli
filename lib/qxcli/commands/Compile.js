  ...
  members: {
    ...
    process: async function() {
      ...
      if (config.typescript) {
        var outputTo = "qooxdoo.d.ts";
        if (typeof config.typescript == "object") {
          if (config.typescript.outputTo)
            outputTo = config.typescript.outputTo;
        }
        if (outputTo[0] != '.')
          outputTo = path.join(maker.getTarget().getOutputDir(), outputTo);
        ts = new qxcompiler.TypeScript(maker.getAnalyser()).set({ outputTo: outputTo });
        await ts.load();
      }
