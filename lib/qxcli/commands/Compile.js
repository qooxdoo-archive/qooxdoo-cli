...
qx.Class.define("qxcli.commands.Compile", {
  ...
  members: {
    ...
    process: function() {
      var t = this;
      this.parse(this.argv, function(err, config) {
        if (err)
          throw new Error("Error: " + err);
        if (!config)
          throw new Error("Error: Cannot find any configuration");
        t.createMakerFromConfig(config, function(err, maker) {
          if (err)
            throw new Error("Error: " + err);
    ...
    parse: function(argv, cb) {
      ...
      var config = {};
      if (parsedArgs.config) {
        this.loadConfig(parsedArgs.config, function(err, data) {
          if (err)
            return cb(err);
          config = data;
          mergeArguments();
    ...
    loadConfig: function(path, cb) {
      ...
      if (path.match(/\.js$/)) {
        fs.exists(path + "on", function (exists) {
          if (exists) {
            qxcli.commands.Compile.loadJson(path + "on", function (err, data) {
              if (err)
                return cb(err);
              loadJs(path, data, cb);
            });
        ...
        qxcli.commands.Compile.loadJson(path, cb);
      }
    },
    ...
    configure: function(path, cb) {
      ...
      return new Promise((resolve,reject)=>{
          t.loadConfig(path, function(err, data) {
            if (err) {
              if (cb) return cb(err);
              return reject(err);
            }
            ...
            t.createMakerFromConfig(data, (err,maker)=>{
              if (err) {
                if (cb) return cb(err);
                return reject(err);
              }              
              if(cb) return cb(null,maker);
              resolve(maker);
            });
          });        
      });
