...
qx.Class.define("qxcli.commands.Compile", {
  ...
  members: {
    ...
    process: async function() {
      if (this.argv["machine-readable"])
        ...
        qxcompiler.Console.getInstance().setWriter((str) => this.__gauge.show(str));
      }
      ...
      // Continuous make
      return new qxcli.Watch(maker).start();
    },
    ...
    createMakerFromConfig: async function(data) {
      ...
      if (!outputPath)
        throw new Error("Missing output-path for target " + data.target.type);
      
      maker = new qxcompiler.makers.AppMaker();
      if (this.argv["no-erase"])
      ...
      data.applications.forEach(function(appData, appIndex) {
        ...
        if (parts) {
          if (!parts.boot)
            throw new Error("Cannot determine a boot part for application " + (appIndex + 1) + " " + (appData.name||""));
    ...
    getYargsCommand: function() {
      return {
        ...
        builder   : {
          ...
          "no-erase": {
            describe: "Blocks automatic deletion of the output directory when compiler version changes",
            boolean: true
          }
        },
  ...
  defer: function(statics) {
    qxcompiler.Console.addMessageIds({
      "qxcli.compile.writingApplication": "Writing application '%2' to %1",
    });
