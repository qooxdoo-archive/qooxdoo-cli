...
qx.Class.define("qxcli.commands.Upgrade", {
  ...
  members: {
    ...
    process: async function() {
      ...
      try {
        ...
        if (ex.code == "ENOENT")
          return null;
        t.error("Cannot read config.json: " + (ex.message||ex.stack))
        return null;
      }
      try {
        configJson = parseJson(data);
      }catch(ex) {
        t.error("Cannot parse config.json: " + (ex.message||ex.stack));
        return null;
      }
      ...
      if (data) {
        try {
          ...
          t.error("Cannot parse compile.json: " + (ex.message||ex.stack));
          return null;
        }
      }
      ...
      if (compileJson.libraries) {
        compileJson.libraries = compileJson.libraries.map(function(path) {
          if (typeof path === "object" && typeof path.path === "string") {
            ...
            throw new Error("Cannot interpret configuration for libraries");
          }
      ...
      if(!libLookup["qx"]) {
        ...
        if (!dir) {
          t.error("Cannot find Qooxdoo in " + dir);
        } else {
  ...
  statics: {
     ...
     */
    yargsInstall: function(module) {
      module.command = "upgrade [options]";
      module.desc = "upgrades a qooxdoo application";
      module.usage = "upgrade";
      module.builder = function (yargs) {
        return yargs
          .option('v', {
              alias: 'verbose',
