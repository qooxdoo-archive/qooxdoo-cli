 ...
 * with some metadata. 
 */
qx.Class.define("qxcli.commands.contrib.Create", {
  extend: qxcli.commands.Contrib,

  statics: {
    getYargsCommand: function() {
      return {
        command: 'create_ <application namespace> [options]',
        ...
        handler: function(argv) {
          return new qxcli.commands.contrib.Create(argv)
            .process()
            .catch((e) => {
  ...
  members: {
    ...
    process: async function() {
      ...
      if ( ! fs.existsSync( skeleton_dir ) ) {
        this.exit(`Application type ${argv.type} has not been implemented yet.`);
      }
          ...
          if (stats.isFile() ) {
            ...
            if ( sourceFile.includes(".tmpl") ) {
              ...
              if( fs.existsSync(targetFile) ){
                this.exit(`${targetFile} already exists.`);
              }
            ...
            traverseFileSystem(sourceFile,targetDir);
          }
        }
      }

      // go
      traverseFileSystem( skeleton_dir, appdir ); 
    }
  }
});
