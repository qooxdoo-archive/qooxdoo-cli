  ...
  members: {
    ...
    process: async function() {
      ...
      if ( repos_cache.list.length == 0 ){
        console.warn("You need to call 'qx contrib update' first.");
      }
      ...
      for ( let repo_name of list) {
        ...
        for (let tag_name of tag_names){
          ...
          for ( let { qx_versions, info } of manifests ){
            if( info === undefined ){ 
              console.warn( `Error: Undefined info field for ${repo_name}/${tag_name}`);
              continue;
            }
            let isCompatible = semver.satisfies(qooxdoo_version, qx_versions, true);
            releaseInfo.push(`  - ${info.name} ${isCompatible?'':'(incompatible, requires qooxdoo ' + qx_versions +')'}`);
        ...
        if( releaseInfo.length && (argv.all || list_repo_only ) ) console.info(releaseInfo.join("\n"));
      }
      this.saveCache();
    }
  }
