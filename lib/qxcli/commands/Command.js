...
qx.Class.define("qxcli.commands.Command", {
  ...
  members: {
    ...
    getAppQxPath : async function(){
      ...
      if ( ! qxpath ) {
        throw new Error( "Cannot determine qooxdoo path." );  
      }
    ...
    getUserQxVersion : async function(){ 
      ...
      let qxversion = await fs.readFileAsync( path.join( path.dirname(qxpath), 'version.txt' ), 'utf-8' );
      if( ! semver.valid( qxversion) ){
        throw new Error(`Invalid qooxdoo version ${qxversion}. Must be valid semver string.`);
      }
      return qxversion;
    },
     ...
     * @deprecated Use {@link #getUserQxVersion} instead.
